import {
  ...
  import { ID, ImageGravity, Query, QueryTypes } from "appwrite";
import { OAuthProvider } from 'appwrite';
export function signInAccount({ provider }: { provider: OAuthProvider }) {
    try {
...
export async function emailSignIn({ email, password }: { email: string; password: string }) {
    try {
  ...
  export const emailSignUp = async ({ email, password, name}: {email: string, password: string, name: string}) => {
    try {
